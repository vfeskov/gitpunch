...
describe('githubAtom', () => {
  describe('fetchAtom', () => {
    ...
    it(`succeeds after ${FETCH_ATTEMPTS - 1} 5xx errors and then a success response`, async () => {
      nock(host)
        .get(path).times(FETCH_ATTEMPTS - 1).reply(500)
      ...
      expect(timeout.testTimePassed()).to.eql(DELAY_BETWEEN_ATTEMPTS * (FETCH_ATTEMPTS - 1))
      expect(totalRequests()).to.eql(FETCH_ATTEMPTS)
    })

    it(`rejects after ${FETCH_ATTEMPTS} 5xx errors`, async () => {
      nock(host)
        .get(path).times(FETCH_ATTEMPTS).reply(500)
        .get(path).reply(200, goodResponse)
      try {
      ...
      expect(timeout.testTimePassed()).to.eql(DELAY_BETWEEN_ATTEMPTS * FETCH_ATTEMPTS)
      expect(totalRequests()).to.eql(FETCH_ATTEMPTS)
    })
